...
class ClassRegistry:
    ...
    async def register_class(self, class_request: FunctionRequest) -> Dict[str, Any]:
        ...
        log.info(
            f"Registered class {class_name} with {len(endpoint_methods)} HTTP endpoints"
        )
    ...
    async def execute_method(
        self, class_name: str, method_name: str, request_data: Dict[str, Any]
    ) -> Any:
        """
    ...
    async def execute_class_method(self, request: FunctionRequest) -> Any:
        ...
        try:
            ...
            log.error(
                f"Remote method execution failed for {class_name}.{method_name}: {e}"
            )
    ...
    async def _get_or_create_instance_with_constructor(
        self, class_name: str, cls: Type, request: FunctionRequest
    ) -> Any:
        ...
        if not request.create_new_instance and instance_id in self.class_instances:
    ...
    def _update_instance_metadata(self, instance_key: str):
        ...
        if instance_key in self.instance_metadata:
            ...
            self.instance_metadata[instance_key]["last_used"] = (
